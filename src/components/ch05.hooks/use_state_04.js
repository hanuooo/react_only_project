...
function App(){
    ...
    const [members, setMembers] = useState([
        ...
        // <li> 태그의 항목을 더블클릭하면 해당 행(row)을 삭제해주는 함수입니다.
    ...
    const AddMember = (event) => {
        // members 배열에서 id 항목들만 추철합니다.
        // 이 결과 역시 객체로 되어 있는데, ... 연산자를 사용하여 스칼라 데이터로 만든 다음
        // max() 메소드를 이용하여 최대 값을 추출합니다.
        const newNextId = Math.max(...members.map((bean)=> bean.id)) + 1 ;

        // 신규 id와 입력한 데이터를 이전 배열 member에 concat() 메소드를 이용하여 추가합니다.
