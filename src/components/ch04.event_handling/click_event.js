function App(){
    ...
    const ClickEvent05 = (event) => {
        console.log(`해당요소의 id : ${event.target.id}`);
        ...
        /* 해당 이벤트가 보유하고있는 속성(Property) 목록을 저장할 배열입니다. */
        const eventArray = [];
    ...
    return(
        <div className="App">
            ...
            <button id ="register"onClick={(event)=>ClickEvent05('hello', event)}>등록</button>
            <button id = "cancel" onClick={(event)=>ClickEvent05('hello', event)}>취소</button>
            <br />
        </div>
    );
}
