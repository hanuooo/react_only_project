...
function App(){
   ...
   /* mode : 현재 상태의 모드 지정 */
   /* insert, update, delete, read, detaill 등등 */
   const [mode, setMode] = useState('');

   /* 프로그램 최초 시작시 1번이 선택되었다고 가정합니다. */
   const [selectedId, setSlelctedId] = useState(1);

   /* 상품 목록에서 특정 상품 1개를 클릭하였습니다. */
   const ClickArrived = (id) =>{
        console.log(`상품 아이디 : ${id}`);
        setSlelctedId(Number(id)) ; // 선택한 상품의 id가 변경 되었습니다.
   ...
   const getProductById = () =>{
        ...
        // selectedProduct는 객체 1개를 저장하고있는 배열로 반환됩니다.
        // 따라서, 1번째 항목은 [0]으로 가져와야 합니다.
    ...
    return(
        <Card>
            ...
            <Card.Body>
                {/* onClickContent 프롭스가 리턴되고 난 후 {ClickArrived} 함수가 동작되도록 하겠습니다. */}
                <Content contents={products} onClickContent={ClickArrived} />
            </Card.Body>
               <Card.Body>
