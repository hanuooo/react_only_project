...
function App({contents, onClickContent}){

    /* 테이블 특정 행(row)의 셀 1개를 클릭했습니다. */
    const ClickItem = (event) => {
        /* 선택한 항목의 부모 요소의 id를 읽어서 할당합니다. 한개,한개를 Node 라고함 */
        const itemId = event.target.parentNode.id ; // 상품 아이디
        console.log(`선택된 상품 id : ${itemId}`);

        onClickContent(itemId); /* 상위 컴포넌트에 해당 id를 넘겨 줍니다. */
    }
        ...
        return(
            <tbody>
                {contents.map((item, index)=>(
                    // tr = 부모 , td = 자식 , 부모에서 지정 , onClick은 자식에서 일일히 해야함
